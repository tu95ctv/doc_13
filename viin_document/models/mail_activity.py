...
class MailActivityType(models.Model):
    ...
    tag_ids = fields.Many2many('viin_document.tag')
...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        if attachment_ids:
            for record in self:
                document = self.env['viin_document.document'].search([('request_activity_id', '=', record.id)], limit=1)
                if document and not document.attachment_id:
                    self.env['viin_document.document'].search([('attachment_id', '=', attachment_ids[0])]).unlink()
                    if not feedback:
    ...
    def create(self, values):
        ...
        if activity_type.category == 'upload_file' and activity.res_model != 'viin_document.document':
            if activity_type.folder_id:
                self.env['viin_document.document'].create({
